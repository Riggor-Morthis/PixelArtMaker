...
public class PaletteCreator {
    ...
    private static void mainLoop() {
        ...
        for (int p = 0; p < _paletteSize; p++) {
            pixelValues = checkEveryPixel(p, p + 1);
    ...
    private static float[] checkEveryPixel(int luminosity1, int luminosity2) {
        float[] pixelValues = new float[4];
        float[] pixelBuffer;

        for (int x = 0; x < _importedPicture.getWidth(); x++) {
            for (int y = 0; y < _importedPicture.getHeight(); y++) {
                if (_importedLuminosity[x][y] == luminosity1 ||
                        _importedLuminosity[x][y] == luminosity2) {
